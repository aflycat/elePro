...
function getWarnUserData() {
	...
	function _UserSuccessStatus(dt) {
		if(dt.HttpStatus == 200 && dt.HttpData.data != null) {
			...
			//根据时间周期查询投诉记录
			JQajaxo("post", "/api/Elevator/get_dtComplainList", true, _initWarnData, _initWarnSuccess);

			function _initWarnSuccess(dt) {
				console.log(dt);
				if(dt.HttpStatus == 200 && dt.HttpData.data != "") {
					...
					for(var i = 0; i < result.length; i++) {
						...
						strData += "<li onclick='goWarnDetail(\"" + result[i].Id + "\")'><div class='complain-list-header'>" +
						...
						if(result[i].LngLat != "" && result[i].LngLat != null) {
							strData += "<div class='complain-list-info'>" +
								...
								"<div class='equip-mapinfo'><span onclick='checkWarnPositionInfo(\"" + result[i].Id + "\")'>查看地图</span></div>" +
								"</div>" +
								"</li>";
						} else {
							strData += "<div class='complain-list-info'>" +
					...
					$("#warnListId ul").html(strData);
				} else {
					$("#warnListId ul").html("<p style='text-align:center;color:#C2C2C2'>查无记录</p>");
				}
			}
		}
	}
...
function goWarnDetail(taskId){
	$("#taskId").val(taskId)
    $("#tableName").val('Dt_Fault')
	mainView.router.loadPage("record/record_complain_detail.html");
}

//根据经纬度显示当前位置
function checkWarnPositionInfo(Id) {
	$("#recordPoitionLng").val(Id);
	mainView.router.loadPage("record/record_warn_map.html");
}
