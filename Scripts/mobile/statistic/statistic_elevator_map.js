...
function statistic_elevator_map() {
	var elevatorId = $("#elevatorId").val();
	var _ElevatorMapData = {
		Id: elevatorId
	}

	JQajaxo("post", "/api/Elevator/get_dtElevatorDetail", true, _ElevatorMapData, _ElevatorMapSuccess);
	//获取电梯详细信息
	function _ElevatorMapSuccess(dt) {
		if(dt.HttpStatus == 200 && dt.HttpData.data != "") {
			var resultData = dt.HttpData.data;
			var centerPositionArrs = [];
			// 百度地图API功能
			var map = new BMap.Map("statisticElevatorMapId");
			// 初始化地图,设置中心点坐标和地图级别
					...
					for(var i = 0; i < resultData.length; i++) {
						(function(x) {
							...
							if(contrastTime(resultData[i].ContractEmp) == "0") {
								//设置维保人员图标控件
								myIcon = new BMap.Icon("/Image/elevator/Marker_big_warn.png", new BMap.Size(64, 64));
								opts = {
									width: 260, // 信息窗口宽度
									height: 120, // 信息窗口高度
									offset: {
								...
								content = "<div style='height:120px;font-size:12px;line-height:20px;'><font style='font-size:14px;color:#E43820'><i class='iconfont icon-web-zuoyebaojing' style='font-size:24px;'></i>" +
									...
									"维保公司：" + resultData[i].maintCompName + "<br>" +
									"维保公司：" + resultData[i].PropertyCompName + "<br>" +
									"合同有效时间：" + timeSplit(resultData[i].ContractEmp) + "<br>" +
									"下次检验时间：" + timeSplit(resultData[i].NextMaint) + "</div></div>";
							} else {
								//设置维保人员图标控件
								myIcon = new BMap.Icon("/Image/elevator/Marker_big.png", new BMap.Size(64, 64));
								opts = {
									width: 260, // 信息窗口宽度
									height: 120, // 信息窗口高度
									offset: {
