function record_complain_map() {
	var recordPosition = $("#recordPoitionLng").val();
	var _initComplainMapData = {
		sql: "select top 10 a.*,b.DeviceId,b.InstallAddr,b.LngLat from Dt_Complaint a left join Dt_Equip b on a.EquipId=b.Id where a.Id=" + recordPosition + " order by a.SubmitTime desc",
	...
	function _initComplainMapSuccess(data) {
		...
		$(data).find("shen").each(function(i) {
			...
			// 百度地图API功能
			var map = new BMap.Map("recordComplainMapId");
			// 初始化地图,设置中心点坐标和地图级别
					...
					if(loadNum == 0) {
						...
						if(ResultInfo == 1) {
							content = "<div style='height:120px;font-size:12px;line-height:20px;'><font style='font-size:14px;color:#1E88FE'><i class='iconfont icon-web-zuoyebaojing' style='font-size:24px;'></i>" +
								"<span style='position:relative;top:-2px;left:3px'>投诉信息</span></font><hr style='height:1px;border:none;border-top:1px solid #EFEFEF;'>" +
						...
						//隐藏底部百度logo
						$('#recordComplainMapId div.anchorBL').hide();
						//调整到最佳视野
